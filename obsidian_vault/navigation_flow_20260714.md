...
```mermaid
flowchart TD
    BS["🛰️ RTK Base Station\nSkytraq PX1172RH\nrtkbase host"]
    WIFI["WiFi Backhaul\nMofi 4500 → GL-iNet Opal\n(RTCM corrections)"]
    F9P_POS["F9P — Position\nRear axle center\n= base_link\nGGA @ 10 Hz"]
    F9P_HDG["F9P — Heading\n~36 in ahead of base_link\nRELPOSNED @ 10 Hz"]
    RTCM["rtcm_server.py\nUDP 6002 broadcast @ 10 Hz\n{lat, lon, heading_deg,\nfix_quality, headValid}"]
    MISSION["Mission File\n.txt — one waypoint/meter\nlat  lon  yaw  lookahead  speed"]
    PP["pure_pursuit_controller.py\nSelects lookahead waypoint\nComputes yt (lateral offset)\nδ = atan2(2·yt·L, ld²)"]
    NORM["Normalize steering\nangular_z = δ / max_steer\n+1.0=full LEFT  −1.0=full RIGHT\nlinear_x = speed m/s (fwd +)"]
    UDP6004["UDP 6004 @ 20 Hz\n{linear_x, angular_z, timestamp}"]
    BRIDGE["teensy_serial_bridge.py\nFormats CMD,linear_x,angular_z\nUSB serial @ 460800 baud"]
    TEENSY["Teensy 4.1\ncontrolSteering() mode 0"]
    MAPSTEER["mapNormalizedSteer(angular_z)\nTwo-segment asymmetric map\n0.0 → pot 447 center\n+1.0 → pot 815 full left\n−1.0 → pot 197 full right"]
    PID["Steering PID\nsteer_setpoint vs steer_current\nanalogRead(STEER_POT_PIN)"]
    IBT2["IBT-2 H-Bridge\nLPWM pin 6 → left\nRPWM pin 5 → right\n25A fused supply"]
    MOTOR["Hydraulic Steering Motor\n→ Front wheels"]
    TRANS["controlTransmission() mode 0\nmpsToJrkTarget(linear_x)\nSPEED_CAL table interpolation"]
    JRK["JRK G2 21v3\nLinear actuator controller"]
    HYDRO["Hydrostatic Transmission\n→ Ground speed"]
