...
def parse_rtcm_message(data, pos):
    ...
    total_length = 3 + length + 3  # Header (3) + Body (length) + CRC (3)
    ...
    if calculated_crc != received_crc:
        logging.warning(f"CRC mismatch at buffer pos {pos}, skipping.")
        return None, None, pos + 1  # Skip to next byte to avoid getting stuck

    # Extract message type (12 bits) from bytes 3-4
    if len(message) < 5:  # Ensure we have at least 2 body bytes
        return None, None, pos + 1
    msg_type = struct.unpack('>H', message[3:5])[0] >> 4  # 12-bit message type
    new_pos = pos + total_length  # Move to end of message
            ...
            if data:
                ...
                while pos < len(rtcm_buffer):
                    ...
                    if message_type is None:
                        pos += 1  # Move to next byte if incomplete/invalid
                        continue
