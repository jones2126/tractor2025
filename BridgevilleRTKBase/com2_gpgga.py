...
try:
    ...
    if ser.is_open:
        ...
        # Send the command to log GPGGA on COM2
        command = "LOG COM2 GPGGA ONTIME 1\r\n"
        ser.write(command.encode('ascii'))
        print(f"\nSent command: {command.strip()}")

        # Read the response to the LOG command (up to 2 seconds)
        print("Reading response to LOG command...")
        start_time = time.time()
        while (time.time() - start_time) < 2:
            if ser.in_waiting > 0:
                response = ser.readline().decode('ascii', errors='ignore').strip()
