...
3. Open the printed dashboard URL on the phone. Voice guidance can help locate **W1**. The launcher requires RTK Fixed, fixed-carrier heading, baseline 0.80–1.30 m, heading accuracy at most 1°, position within 1.50 m of W1, and heading within 20°. It also runs the mission preflight, including firmware identity and neutral/safe-mode checks.
...
5. After any GPS/heading or radio safety loss, this launcher does **not** automatically resume. Keep Pause selected, diagnose the fault, and require an intentional handheld mode cycle after the system is healthy. If uncertain, end the run.
...
The prior field test lost NRF24 connectivity and RTK fix. If either happens again, stop and investigate rather than attempting to finish the route. The recorded source trace contains 22 DGPS samples; the 0.15 m resampling does not improve their original positional accuracy.
