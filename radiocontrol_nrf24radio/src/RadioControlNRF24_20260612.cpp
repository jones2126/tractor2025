...
const int modeSwitchPin3 = 3;
const int modeSwitchPin4 = 4;
...
void checkModeSW() {
    ...
    if (currentMillis - lastModeCheck >= modeCheckInterval) {
        ...
        int pin3State = digitalRead(modeSwitchPin3);   // Physical pin 3
        int pin4State = digitalRead(modeSwitchPin4);  // Physical pin 4        
        ...
        if (pin4State == LOW && pin3State == HIGH) {
          radioData.control_mode = 2;  // UP position for Pause
        }
        else if (pin3State == LOW && pin4State == HIGH) {
          radioData.control_mode = 0;  // DOWN position for Auto
        }
        else if (pin3State == HIGH && pin4State == HIGH) {
          radioData.control_mode = 1;  // MIDDLE position for Manual
        }
