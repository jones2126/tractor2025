...
// continuous target, so auto mode interpolates this table instead of
// quantizing to the RC joystick buckets above.
//
// !! PLACEHOLDER anchors: assumes full forward (2288) = 1.5 m/s with a
// linear ramp from neutral. Replace with measured {m/s, target} pairs
// from the speed-calibration field test (command targets, time a 10 m
...
void setup() {
    ...
    Serial.println("1,0,SYS,start,fw=teensy_main_20260804,steer_hz=20");
    Serial.flush();
}
