#!/usr/bin/env python3
"""
server_step4.py — WebRTC video + telemetry + bidirectional controls (DepthAI v2.x)
UI changes:
  - Larger video (960x540)
  - Steering slider placed under video
  - Steering presets like Tkinter
  - Speed control as radio buttons (shows current selection)
  - Big red E-STOP near top (sticky header)

Run:
  python3 ~/tractor2025/tractor/webrtc/server_step4.py
Open:
  http://<PI_IP>:8080/
"""
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  header{position:sticky;top:0;background:#141414;display:flex;align-items:center;gap:12px;padding:10px 16px;border-bottom:1px solid #222;z-index:10}
  ...
  .label{color:var(--muted);min-width:150px}
  ...
  .radio-group{display:grid;grid-template-columns:1fr;gap:6px}
  ...
  <header>
    ...
    <div id="conn">Status: <span id="connState">init</span></div>
  </header>

  <div class="wrap">
    <!-- Left: Video + Steering -->
    <div class="card">
      <h2 style="margin:0 0 8px 0;">Video</h2>
...
pc.oniceconnectionstatechange = ()=> { log('iceConnectionState:', pc.iceConnectionState); connStateEl.textContent = pc.iceConnectionState; };
pc.onconnectionstatechange = ()=> { log('connectionState:', pc.connectionState); connStateEl.textContent = pc.connectionState; };
...
document.getElementById('neutral').onclick = ()=> {
  ...
  steer.value = 0.0; steer.dispatchEvent(new Event('input'));
  sendCtrl({ speed_pos: 4, steering: 0.0 });
};
