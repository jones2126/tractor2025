#!/usr/bin/env python3
"""
server_step6.py – WebRTC video + telemetry + bidirectional controls (DepthAI v2.x)
Updated layout:
  - Video, steering presets, and slider all match in width
...
class ServoController:
    ...
    def _speed_pos_to_angle(position:int)->float:
        if position <= 4:
            angle = 90 + ((position - 4) * 30)
        else:
            angle = 90 + ((position - 4) * 15)
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  header{background:#141414;display:flex;justify-content:flex-end;align-items:center;gap:10px;padding:6px 12px;border-bottom:1px solid #222;z-index:10}
