...
def generate_frames():
    ...
    while True:
        ...
        # BGR for vibrant light blue: (200, 220, 255)
        overlay_width = 500
        overlay_height = 130
        left = 15  # Top-left horizontal position
        top = 140  # Moved down from 50 to 100 pixels from top
        right = left + overlay_width
        bottom = top + overlay_height
        # cv2.rectangle(frame, (left, top), (right, bottom), (200, 220, 255), -1)  # light tan
        ...
        cv2.putText(frame, "SPEED CONTROL", (left + 30, top + 50), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)  # Black text
        
        pos_mode_text = f"Position {current_speed_position}, Mode: {get_speed_description(current_speed_position)}"
        cv2.putText(frame, pos_mode_text, (left + 30, top + 100), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)  # Black text
...
def index():
    ...
    <html>
    <head>
        <title>Working Light Blue Overlay + Speed Control</title>
