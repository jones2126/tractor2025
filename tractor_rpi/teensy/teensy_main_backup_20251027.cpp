...
// Add globals to test receiving cmd_vel data
...
// Standardized serial print functions
...
void checkCmdVelTimeout() {
    if (cmdVel.received && (millis() - last_cmd_vel_time > CMD_VEL_TIMEOUT)) {
        cmdVel.linear_x = 0.0; // Fixed typo: 'cmd lad' to 'cmdVel'
        cmdVel.angular_z = 0.0;
        cmdVel.received = false;
        printStatus("CMD_VEL", "timeout_reset_to_zero");
    }
}
...
void debugSerialBuffer() {
    ...
    if (currentMillis - lastDebug >= 1000) {
        printDebugKV("SERIAL", "available", Serial.available());
