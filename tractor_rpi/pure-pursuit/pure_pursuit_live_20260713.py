"""
pure_pursuit_live_20260713.py -- Pure Pursuit controller with live F9P/GPS input.

Based on pure_pursuit_grok_20251112.py. Change log vs that file:

  NEW   GPSReceiver class -- background thread, listens on UDP 6002 (rtcm_server's
        broadcast), parses {lat, lon, heading_deg, fix_quality, headValid, carrier,
        fatal_error, ...}, converts compass heading_deg -> math-frame radians
        (CCW from east) via radians(90 - heading_deg), and exposes the latest pose
        with an "age" so staleness can be checked every cycle.
  NEW   PurePursuit.run_live() -- new mode. Every cycle: pulls the latest pose,
        refuses to drive (sends a stop) if the pose is stale, fix_quality doesn't
        meet --min-fix, headValid is False, or rtcm_server reports fatal_error.
        Otherwise computes steering and sends it.
  NEW   angle_to_normalized() -- steering is sent to the bridge as a NORMALIZED
        value: +1.0 = full LEFT lock, -1.0 = full RIGHT lock, 0.0 = center.
        The Teensy firmware (auto mode 2) owns the mapping from normalized
        steering to its machine-specific pot calibration (197/447/815 on
        tractor01). Machine calibration stays out of the navigation layer.
        !! REQUIRES the matching Teensy firmware change -- do not run against
        firmware that still expects raw pot counts (0-1023) in angular_z.
  NEW   speed_to_linear_x() -- converts m/s to the bridge's normalized -1..+1
        linear_x (negative = forward, per teensy_serial_bridge convention).
        The original script sent raw m/s directly, which the bridge would have
        read as a near-zero *reverse* command.
  CHANGED cleanup() now sends one explicit stop command (linear_x=0, steer=0)
        before closing the socket, instead of relying solely on the bridge's
        own 1 s cmd_vel timeout.
  CHANGED compute_steering(): end-of-path circle-intersection root selection
        uses copysign(sqrt_disc, v) (direction of travel) instead of
        copysign(sqrt_disc, rel_x) (sign of position), matching the original
        C++ pure_pursuit.cpp's copysign(D, v_).
  CLEANED gps_offset_y: removed the contradictory comments/double assignment
        from the original; single clear value.

Everything else (load_path, latlon_to_xy, gps_to_base, the core Pure Pursuit
math in compute_steering) is carried over as-is from pure_pursuit_grok_20251112.py.

cmd_vel JSON sent on UDP 6004:
    {"linear_x": <-1..0, negative=forward>,
...
class PurePursuit:
    def __init__(self, wheelbase=1.27, max_steer=0.623, pos_tol=0.1,
        ...
        :param max_speed_mps: UNVERIFIED placeholder -- the ground speed that
               corresponds to a full-scale (|linear_x|=1.0) command. Bench/field
               calibrate this (command a fixed linear_x, measure actual m/s)
               before trusting speed_to_linear_x().
        """
        self.L = wheelbase
        ...
        self.gps_offset_y = 0.1524   # 6 inches left, in meters (+y = left)
    ...
    def speed_to_linear_x(self, v_mps):
        """Speed (m/s, always >= 0 in our mission files) -> bridge's normalized
        linear_x, where NEGATIVE = forward (teensy_serial_bridge convention:
        -1.0=full fwd, 0.0=neutral, +1.0=full rev).
        !! self.max_speed_mps is an UNVERIFIED placeholder -- calibrate it
        (command a fixed linear_x, measure actual ground speed) before a real run.
        """
        normalized = v_mps / self.max_speed_mps if self.max_speed_mps > 0 else 0.0
        normalized = max(0.0, min(1.0, normalized))
        return -normalized

    # -- UDP send / stats ---------------------------------------------------------

    def send_cmd_vel(self, linear_x, angular_z_normalized):
        cmd = {
            'linear_x': linear_x,
    ...
    def _send_stop(self):
        """Explicit neutral speed + centered steering."""
        try:
    ...
    def run_interactive(self):
        ...
        while self.running:
            try:
                ...
                linear_x = self.speed_to_linear_x(v)
                self.send_cmd_vel(linear_x, steer_cmd)
                ...
                print(f"Target idx={self.idx}/{len(self.path)}, delta={math.degrees(delta):.2f}deg, "
                      f"steer={steer_cmd:+.2f}, v={v:.2f}m/s, linear_x={linear_x:.2f}")
            ...
            while self.running:
                ...
                if current_time >= next_send_time:
                    ...
                    if self.messages_sent % 100 == 0:
                        print(f"Sent #{self.messages_sent}: v={v:.2f}, steer={steer_cmd:+.2f} "
                              f"(delta={math.degrees(delta):.1f}deg), idx={self.idx}")
                ...
                if not ok:
                    ...
                    if loop_count % 20 == 0:
                        print(f"idx={self.idx}/{len(self.path)} x={self.last_x:.2f} y={self.last_y:.2f} "
                              ...
                              f"steer={steer_cmd:+.2f} v={v:.2f} linear_x={linear_x:.2f} "
...
def main():
    ...
    parser.add_argument('--max-speed', type=float, default=1.0,
                        help='UNVERIFIED placeholder: m/s at |linear_x|=1.0. Calibrate before trusting this.')
