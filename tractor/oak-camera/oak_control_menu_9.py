...
last_steering_command_time = None  # Track the time of the last steering command
last_steering_value = None  # Track the last steering value sent
...
def update_speed_servo(position):
    ...
    # Map position 1 (reverse max) to 0°, 4 (neutral) to 90°, 10 (forward max) to 180°
    if position <= 4:
        # Reverse: 1=0°, 4=90°
        angle = 90 + ((position - 4) * 30)  # Each reverse step: -30° from neutral
    else:
        # Forward: 5=120°, 10=180°
        angle = 90 + ((position - 4) * 15)  # Each forward step: +15° from neutral (finer for forward)
...
def generate_frames():
    ...
    global latest_frame, current_speed_position, current_steering, last_steering_command_time, last_steering_value
    ...
    while True:
        ...
        overlay_width = 500
        overlay_height = 200  # Increased height for latency display
        ...
        # Telemetry text - adjusted y positions to include latency
        ...
        # Calculate latency based on steering angle update
        latency = 0.0
        if last_steering_command_time is not None and last_steering_value is not None:
            current_time = time.time()
            if current_steering != last_steering_value:  # Only calculate when value changes
                latency = current_time - last_steering_command_time
                last_steering_command_time = None  # Reset after calculation
            latency_text = f"Latency: {latency:.2f}s"
        else:
            latency_text = "Latency: 0.00s"
        cv2.putText(frame, latency_text, (left + 30, top + 160), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)  # Smaller latency text
    ...
    <body>
        ...
        <script>
            ...
            function setSteering(value) {
                ...
                const command_time = new Date().getTime() / 1000; // Convert to seconds
                socket.emit('steering_update', {value: value, timestamp: command_time});
                console.log('Emitted steering_update with value:', value, 'at:', command_time);
            }
...
def handle_speed_update(data):
    ...
    try:
        ...
        update_speed_servo(current_speed_position)  # Update speed servo
...
def handle_steering_update(data):
    ...
    global current_steering, last_steering_command_time, last_steering_value
    try:
        current_steering = float(data['value'])
        last_steering_command_time = float(data['timestamp'])  # Store the command timestamp
        last_steering_value = current_steering  # Store the last sent value
        update_steering_servo(current_steering)  # Update steering servo
        print(f"Steering set to: {current_steering} at {last_steering_command_time}")
    except (KeyError, ValueError) as e:
