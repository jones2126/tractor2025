...
def add_overlays(frame):
    """Add teleoperation overlays to the frame based on global flags"""
    ...
    # ✨ NEW: Correctly check the show_telemetry flag
    if show_telemetry:
            box_width = 380
            box_height = 180
...
def capture_frames_working():
    """Frame capture - just get raw frames without overlays"""
    global latest_frame
                ...
                while True:
                    ...
                    if in_rgb:
                        ...
                        if frame is not None and frame.size > 0:
                            # Just store raw frame - overlays will be added in generate_frames()
                            with frame_lock:
...
def generate_frames():
    """Generate frames for HTTP streaming with guaranteed overlays"""
    global latest_frame
    
    while True:
        if latest_frame is not None:
            with frame_lock:
                # Get a copy of the latest raw frame
                base_frame = latest_frame.copy()
        else:
            # Create a black frame if no camera data
            base_frame = np.zeros((720, 1280, 3), dtype=np.uint8)
            cv2.putText(base_frame, "Waiting for camera...", (50, 360), 
                       cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)

        # ✨ NEW: Call the dedicated overlay function to add all overlays
        add_overlays(base_frame)
        
        # Add timestamp
        height, width = base_frame.shape[:2]
        timestamp = time.strftime("%H:%M:%S", time.localtime())
        cv2.putText(base_frame, timestamp, (width - 150, height - 20), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        # Encode frame as JPEG
        ret, buffer = cv2.imencode('.jpg', base_frame, [cv2.IMWRITE_JPEG_QUALITY, 85])
        if ret:
