...
def add_overlays(frame):
    ...
    if show_telemetry:
        ...
        cv2.putText(frame, f"RTK Fix: {'YES' if robot_status['rtk_fix'] else 'NO'}", (20, y_offset), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0) if robot_status['rtk_fix'] else (0, 0, 255), 2)
        y_offset += 25
        cv2.putText(frame, f"Autopilot: {'ON' if robot_status['autopilot'] else 'OFF'}", (20, y_offset), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0) if robot_status['autopilot'] else (255, 255, 255), 2)
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            h1, h2, h3 { color: #00ff00; text-align: center; margin: 10px 0; }
            h3 { font-size: 16px; }
        </style>
    </head>
    <body>
...
def handle_steering(data):
    ...
    print(f"Steering: {data['steering']:.1f}")
